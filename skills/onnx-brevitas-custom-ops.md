...
- You want to replace standard PyTorch operations (e.g., `Conv2d`, `Linear`) with custom ONNX nodes (e.g., `mydomain::CustomQuantConv`).
...
3. **Implement `symbolic`**: Define `symbolic(g, inputs...)` to construct the ONNX graph. The first argument is the graph builder `g`. Emit the custom node and attach attributes:
   ```python
   return g.op("mydomain::CustomOp", x, weight, bias, attr_i=int(val), attr_s=str(val))
   ```
