...
# ── Step 4: LR finder, starting from the fully PTQ-calibrated model ─────────
# --pretrained is omitted here too: --init-from-ptq supplies everything.
# echo
# echo "[4/5] LR finder (starting from the fully PTQ-calibrated model)..."
# LR_FINDER_LOG="$(mktemp)"
...
#     "${DATA_FLAGS[@]}" \
#     --find-lr 2>&1 | tee "$LR_FINDER_LOG"
# 
# FOUND_LR="$(grep "Steepest descent LR" "$LR_FINDER_LOG" | grep -oE '[0-9.]+e[+-][0-9]+' | head -1)"
# rm -f "$LR_FINDER_LOG"
# if [[ -z "$FOUND_LR" ]]; then
#     echo "[ERROR] Could not parse the suggested LR from the LR finder output." >&2
#     exit 1
# fi
# echo "  -> Found LR: $FOUND_LR"
...
echo "  weight PTQ checkpoint     : $WEIGHTS_CKPT"
echo "  bias PTQ checkpoint       : $BIAS_CKPT"
